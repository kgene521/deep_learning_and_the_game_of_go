...
class DeepLearningAgent(Agent):
    ...
    def select_move(self, game_state):
        ...
        eps = 1e-6
        move_probs = np.clip(move_probs, eps, 1 - eps)  # prevent move_probs from getting stuck at 0 or 1
        move_probs = move_probs / np.sum(move_probs)    # renormalize to get another probability distribution
        candidates = np.arange(num_moves)
        ranked_moves = np.random.choice(                # Turns the move probabilities into a ranked list of moves
            candidates, num_moves, replace=False, p=move_probs) # samples potential candidate moves
